...
def draw_and_spectate_junction_routes_iteratively(client, junction_routes, delay=2.0):
    ...
    for junction_id, routes in junction_routes.items():
        for route_wps in routes:
            ...
            place_spectator_on_transform(client.get_world(), route_wps[len(route_wps) // 2].transform)
...
def spawn_pedestrians_around_wp(client, reference_location: carla.Location, 
                                     num_pedestrians=5, radius=15.0, min_distance=3.0):
    """
    ...
    while len(valid_spawn_points) < num_pedestrians and spawn_attempts < max_attempts:
        ...
        if not too_close:
            valid_spawn_points.append({
                ...
                'distance_from_reference': reference_location.distance(spawn_transform.location)
            })
    ...
    for i, response in enumerate(controller_results):
        if response.error:
            print(f"Failed to spawn controller for pedestrian {pedestrian_actors[i]}: {response.error}")
        else:
    ...
    print(f"Successfully spawned {len(spawned_pedestrians)} pedestrians with AI controllers")
    
    return pedestrian_actors


# Alternative version with waypoint-based spawning for more precise placement
def spawn_vehicles_around_wp(client, reference_location: carla.Location, ignore_traffic_lights=True, num_vehicles=5, search_distance=30.0, search_resolution=3.0):
   ...
   reference_waypoint = carla_map.get_waypoint(reference_location, project_to_road=True, lane_type=carla.LaneType.Driving)
   ...
   for lane_change in [reference_waypoint.get_left_lane(), reference_waypoint.get_right_lane()]:
   ...
   valid_spawn_points = list({v['waypoint'].id: v for v in valid_spawn_points}.values())
   ...
   return spawn_vehicles_from_transforms(client, selected_spawns, ignore_traffic_lights)
...
def spawn_vehicles_from_transforms(client, spawn_data_list, ignore_traffic_lights):
   ...
   safe_blueprints = [x for x in blueprints if int(x.get_attribute('number_of_wheels')) == 4]
   ...
   for spawn_data in spawn_data_list:
       ...
       batch.append(
           SpawnActor(blueprint, spawn_data['spawn_point'])
   ...
   for i, response in enumerate(responses):
       if response.error:
           print(f"Failed to spawn vehicle {i}: {response.error}")
            ...
            set_ignore_traffic_lights(client, world, response.actor_id, ignore_traffic_lights)
...
def place_spectator_on_transform(world, transform, height=60.0, pitch=-90.0):
    ...
    spectator.set_transform(carla.Transform(spectator_location, carla.Rotation(pitch=rotation.pitch, yaw=0, roll=0)))    
...
def draw_waypoints(world, waypoints, life_time=30.0, highlight=None):
    ...
    for waypoint in waypoints:
        ...
        debug.draw_point(
            ...
            color=carla.Color(0, 0, 255) if highlight else carla.Color(0, 255, 0),  # Green
            life_time=life_time
        )
