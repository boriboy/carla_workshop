...
class LiDARSensor(SensorWrapper):
    ...
    def __init__(
        self, world, points_per_second=90000, rotation_frequency=40, range=LIDAR_RANGE_M
    ):
        super().__init__(world)
    ...
    def lidar_to_bev(points, grid_size=256, range_m=LIDAR_RANGE_M):
        ...
        valid_mask = (
            (x_grid >= 0) & (x_grid < grid_size) & (y_grid >= 0) & (y_grid < grid_size)
        )
    ...
    def carla_bev_to_plot(lidar_data, range_m=50):
        ...
        # Visualize
        LiDARSensor.visualize_bev(
            lidar_data.frame, bev, range_m=range_m, title="CARLA LiDAR BEV"
        )
    ...
    def visualize_bev(frame, bev_matrix, range_m=50, title="LiDAR Bird's Eye View"):
        ...
        circle1 = plt.Circle(
            (0, 0), 25, fill=False, color="white", alpha=0.5, linestyle="--"
        )
        circle2 = plt.Circle(
            (0, 0), range_m, fill=False, color="white", alpha=0.5, linestyle="--"
        )
