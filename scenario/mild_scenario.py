...
class MildScenario(ScenarioBase):
    def __init__(self, config, client, map, ego_wrapper, junction_id, route):
        ...
        self._vehicles = spawn_vehicles_around_wp(
            client,
            mid_route_wp.transform.location,
            ignore_traffic_lights,
            num_vehicles=NUM_VEHICLES,
        )
        self._pedestrians = spawn_pedestrians_around_wp(
            client, mid_route_wp.transform.location, num_pedestrians=NUM_PEDESTRIANS
        )
