...
class JunctionTurnArchitecture(nn.Module):
    def __init__(self):
        ...
        self.relative_position_fc = nn.Sequential(
            nn.Linear(3, 16), nn.ReLU(), nn.Linear(16, 32), nn.ReLU()
        )

        # Shared feature layer (CNN features + collision features)
        feature_size = (
            256 * 4 * 4 + 32 + 32
        )  # CNN output + collision features + relative position features
        self.shared_fc = nn.Sequential(
            nn.Linear(feature_size, 512), nn.ReLU(), nn.Linear(512, 256), nn.ReLU()
        )
    ...
    def forward(self, lidar_state, collision_state, relative_position):
        ...
        combined_features = torch.cat(
            (
                lidar_features.flatten(start_dim=1),
                collision_features.flatten(start_dim=1),
                relative_features.flatten(start_dim=1),
            ),
            dim=1,
        )
