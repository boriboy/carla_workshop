...
class ModelTrainer:
    ...
    def train_on_batch(self, episode_data):
        ...
        states_relative_position = torch.stack(
            [exp["state_relative"] for exp in episode_data]
        )
        actions = torch.stack([exp["action"] for exp in episode_data])
        rewards = torch.tensor(
            [exp["reward"] for exp in episode_data], dtype=torch.float32
        )
        ...
        self.model.train()
        action_means, action_stds, values = self.model(
            states_lidar, states_collision, states_relative_position
        )
        ...
        surr2 = (
            torch.clamp(ratio, 1.0 - 0.2, 1.0 + 0.2) * advantages
        )  # 0.2 is clip_param
        ...
        print(
            f"Training - Total Loss: {total_loss.item():.4f}, "
