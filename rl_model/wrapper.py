...
class ModelWrapper(JunctionTurnArchitecture):
    ...
    def forward_pass(
        ...
        ego_wrapper,
        frame: int,
        final_destination_wp,
        reward_function,
        sensor_data_wait_s=0.003,
    ):
        ...
        relative_position_norm, relative_position = (
            ModelWrapper.preprocess_relative_location_and_yaw(
                ego_wrapper.get_ego().get_transform(), final_destination_wp.transform
            )
        )
        ...
        reward = reward_function.calculate_reward(
            vehicle.get_location(), vehicle.get_velocity(), has_collision
        )
    ...
    @staticmethod
    def preprocess_relative_location_and_yaw(
        vehicle_transform, waypoint_transform, max_range=50.0
    ):
        ...
        # relative position
        relative_position_norm = torch.tensor(
            [norm_dx, norm_dy, norm_dyaw], dtype=torch.float32
        ).to(globals.device)
        relative_position = torch.tensor(
            [local_dx, local_dy, delta_yaw], dtype=torch.float32
        ).to(globals.device)
