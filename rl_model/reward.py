...
class WaypointRewardFunction:
    ...
    def calculate_reward(self, vehicle_location, vehicle_velocity, collision_detected):
        ...
        total_reward -= (
            vehicle_location.distance(self.waypoints[-1].transform.location) * 0.1
        )
        ...
        heading_reward = self._calculate_heading_reward(
            vehicle_location, vehicle_velocity
        )
    ...
    def _calculate_progress_reward(self, vehicle_location):
        ...
        distance_to_target = vehicle_location.distance(
        ...
        if self.current_waypoint_idx > 0:
            ...
            distance_from_prev = vehicle_location.distance(
                prev_waypoint.transform.location
            )
    ...
    def _calculate_heading_reward(self, vehicle_location, vehicle_velocity):
        ...
        vehicle_heading = (
            np.array([vehicle_velocity.x, vehicle_velocity.y]) / velocity_magnitude
        )
    ...
    def _check_completion_bonus(self, vehicle_location):
        ...
        if self.current_waypoint_idx >= len(self.waypoints):
            ...
            distance_to_final = vehicle_location.distance(
                final_waypoint.transform.location
            )
