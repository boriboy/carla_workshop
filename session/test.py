...
class TestSession(Session):
    ...
    def callback(self, scenario: ScenarioBase):
        ...
        while scenario.is_running:
            ...
            # forward pass
            try:
                vehicle_control, has_collision, frame_data = self._model.forward_pass(
                    scenario.ego_wrapper,
                    frame,
                    scenario._route[-1],
                    reward_function,
            ...
            # 4. goal reached
            distance_to_goal_m = vehicle.get_location().distance(
                scenario._route[-1].transform.location
            )
            reached_max_steps = (frame - start_frame) >= MAX_SCENARIO_STEPS
            goal_reached = (
                distance_to_goal_m <= SUCCESS_DISTANCE_TO_GOAL
            )  # within 20cm of the goal
            should_terminate = (
                has_collision
                or reached_max_steps
                or distance_to_goal_m > MAX_DISTANCE_TO_GOAL
                or goal_reached
            )
            ...
            if should_terminate:
                ...
                print(
                    f"Termination criteria met: "
                    + (f"collision " if has_collision else "")
                    + (f"max steps reached " if reached_max_steps else "")
                    + (
                        f"distance to goal too high "
                        if distance_to_goal_m > MAX_DISTANCE_TO_GOAL
                        else ""
                    )
        ...
        return (
            goal_reached,
            has_collision,
        )  # return whether the scenario was successful (goal reached)
